...
#         elif len(ts_str)==1:
#             ts_str = '0'+ts_str+'00'
#         ts_str = dt.datetime.strptime(ts_str, '%H%M').hour
        return ts_str
    except Exception as e:
        raise(ts_str)
convertUDF = f.udf(lambda z: fix_crs_dep_time(z),IntegerType())
# df = df.withColumn("CRS_DEP_TIME_HOUR", df["CRS_DEP_TIME"].cast("string"))
# df = df.withColumn("CRS_DEP_TIME_HOUR", convertUDF(col("CRS_DEP_TIME_HOUR")))
...
# CAT_PREDICTORS = set(['MONTH','DAY_OF_MONTH','DAY_OF_WEEK','CRS_DEP_TIME_HOUR']) # GL
CAT_PREDICTORS = set(['MONTH','DAY_OF_MONTH','DAY_OF_WEEK']) # GL
...
#         self.df =  self.df.na.drop() # GL Edit based on testing 
    ...
    def one_hot_encode(self, col_names):
...
class DataSplit():
    ...
    def __init__(self,df,train_percent=0.8,timestamp_col='CRS_DEP_TIME_UTC', folds = 1, simple_splits = False):
        self.df = df
...
#     def cv_folds(self, df = None, folds = 1, train_pct = .8, simple_splits = False):
    def cv_folds(self):
        
        # Assign ID for splitting
        df_ordered = self.df.withColumn('SPLIT_ID', f.row_number().over(Window().orderBy(f.col('CRS_DEP_TIME_UTC_HOUR')))).cache()
        ...
        if self.simple_splits:
            for year in [2015, 2016, 2017]:

                train_df = df_ordered.where(f.col('CRS_DEP_TIME_UTC_HOUR') == year)
                val_df = df_ordered.where(f.col('CRS_DEP_TIME_UTC_HOUR') == (year + 1))
        ...
        # Create folds by number of folds, train/test split %
            ...
            for k in range(self.folds): 
                ...
                # Split the data
                train_df = df_ordered.where(f.col('SPLIT_ID').between(train_ids[0], train_ids[1]))
                val_df = df_ordered.where(f.col('SPLIT_ID').between(val_ids[0], val_ids[1]))
...
model_df = all_features_va.transform(transformer.df).cache()
...
train_dfs, val_dfs = DataSplit(df = model_df, folds = 2, train_percent = .7).cv_folds()
...
for i in range(2):
    print(i)
    train_dfs[i].agg({'SPLIT_ID': 'min'}).show()
    train_dfs[i].agg({'SPLIT_ID': 'max'}).show()
    test_dfs[i].agg({'SPLIT_ID': 'min'}).show()
    test_dfs[i].agg({'SPLIT_ID': 'max'}).show()
...
for fold in range(2):
    Model(train_dfs[fold],val_dfs[fold],label_col=['DEP_DEL15'],feature_col=['ALL_FEATURES_VA'],
                 **{
                     'maxIter':30,
                     'elasticNetParam':1,
                     'regParam':0.01,
